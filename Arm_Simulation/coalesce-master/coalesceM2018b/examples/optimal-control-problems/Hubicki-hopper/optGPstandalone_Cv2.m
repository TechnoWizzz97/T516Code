...
while(~OPT_DONE)
    ...
    t.initialGuess = guess1; %0.1 %1; %0.05;
    t = nlp.addTime;
    t.initialGuess = guess2; %0.1 %1; %0.05;
    T(2) = T(1)+t;
    ...
    nlp.addConstraint(0,ddzf(iC)-(-g - fSpring/mf + fGP/mf),0)
    ...
    nlp.addConstraint(0,fGPInit - (ma+mr+mf)*g,0)
    ...
    % DEBUG. Ddded to ensure no wacky starting points from GP
    nlp.addConstraint(-0.05,zf(iC).initial,0.05)
    % ALSO DEBUG
    nlp.addConstraint(0,fGP,Inf)
    % END DEBUG
    
    
    % initial CoM position
    % nlp.addConstraint(start_height,(ma*zm(iC).initial + mr*zr(iC).initial + mf*zf(iC).initial)/(ma+mr+mf),start_height)
    
    % nlp.addConstraint(rod_initial_height,zr(iC).initial,rod_initial_height)
    ...
    % nlp.addConstraint(-Inf,(ma*dzm(iC).final + mr*dzr(iC).final + mf*dzf(iC).final)/(ma+mr+mf),0)
    ...
    %%%%%
    nlp.addConstraint(0,(dzr(iF).final),0)
    nlp.addConstraint(0,(dzr(iF).final-dzm(iF).final),0)
    nlp.addConstraint(0,(zr(iF).final-zf(iF).final)-(zr(i0).initial-zf(i0).initial),0)
    % nlp.addConstraint(0,dzf(iF).final,0)
    % nlp.addConstraint(0,fSpringFinal,0)
    % nlp.addConstraint(-Inf,(zr(iF) - zr(i0).initial),rod_hop_height)
    ...
    % nlp.addObjective(-zr(iF).final + zr(i0).initial, ...
    ...
    % nlp.addObjective( ...
    % 	(trapz((dzm(1)-dzr(1))'*(dzm(1)-dzr(1)), T(1))+ ...
    % 	trapz((dzm(2)-dzr(2))'*(dzm(2)-dzr(2)), T(2)))*force_weight+ ...
    % 	(trapz(fm(1)'*fm(1), T(1))+ ...
    % 	trapz(fm(2)'*fm(2), T(2))) + ...
    % 	(trapz(ddzd(1)'*ddzd(1), T(1))+ ...
    % 	trapz(ddzd(2)'*ddzd(2), T(2)))*accel_weight, ...
    ...
    for iter_loop = 1:numel(T)
        ...
        fm_vector = [fm_vector, fm_sol(iter_loop,:)];
        
        % mg_vector = [mg_vector, mg_sol(iter,:)];
        ...
        % For animation of ground
        if(iter_loop == 1)
            zg_vector = [zg_vector, zf_sol(iter_loop,:)];
        else
            zg_vector = [zg_vector, zf_sol(iter_loop-1,end)+0*zf_sol(iter_loop,:)];
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % if(PLOT_ON)
    % scene = TerrainHopperScene(response);
    % Player(scene);
    % end
    ...
    % figure
    % hold on
    % plot(eval(z_cone_start)-zf_sol(1,:), mg_sol(1,:), 'b-')
    % % plot(eval(z_cone_start)-zf_sol(2,1), mg_sol(2,1), 'b.')
    % % plot(eval(z_cone_start)-zf_sol(2,:), mg_sol(2,:), 'b-')
    % % plot(eval(z_cone_start)-zf_sol(3,1), gm_sol(3,1), 'b.')
    % % plot(t_sol3, zr_sol(3,:)-zr_init, 'b-')
    % hold off
    ...
    signal_opt = [time_vector(1:end-1); diff(dzm_vector-dzr_vector)./diff(time_vector)];
    max_time = time_vector(end-1);
    ...
    figure
    hold on
    plot(t_sol1, (zr_sol(1,:)-zr_init)*1000+51.42, 'k--')
    plot(t_sol2, (zr_sol(2,:)-zr_init)*1000+51.42, 'k--')
    hold off
    
    figure
    hold on
    plot(t_sol1, squeeze(eval(fGP)), 'm--')
    hold off
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if(OPT_ITER == 1)
        max_hop_height = -eval(nlp.objective)
    end
    
    if(rod_hop_height > max_hop_height)
        rod_hop_height = max_hop_height - mod(max_hop_height,0.005)
    end
    
    
    if(OPT_ITER > 1 && optim.info.status == 0)
        OPT_DONE = 1;
    end
    
    %%%%%%%%%%%%%%%
    if(OPTIM_PAUSING_ON && ~OPT_DONE)
        
        disp('Pausing for keyboard queries...')
        keyboard
        
        userquery = input('quit the optimization loop? 1/0 ');
        if(userquery==1)
            OPT_DONE = 1;
        end
    ...
    if(OPT_ITER > 1 && ~OPT_DONE)
        if(mod(rod_hop_height,0.005) > 0.0001)
            rod_hop_height = rod_hop_height - mod(rod_hop_height,0.005)
        else
            rod_hop_height = rod_hop_height - 0.005
        end
    end
    OPT_ITER = OPT_ITER + 1;
...
if(EXPORT_ON)
    motor_vel_encoder(1,1) = 0;
    
    dlmwrite([robot_directory_name filesep 'updated_trajectory.txt'], [motor_pos_encoder.', motor_vel_encoder.', time_coding.'] , 'delimiter',' ','precision','%0.0f')
    dlmwrite([directory_name filesep  datestr(right_now,'yyyymmddTHHMMSS'), '_robot', '.txt'], [motor_pos_encoder.', motor_vel_encoder.', time_coding.'] , 'delimiter',' ','precision','%0.0f')

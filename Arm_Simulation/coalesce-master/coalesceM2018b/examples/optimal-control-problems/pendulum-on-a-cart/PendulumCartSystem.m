...
classdef PendulumCartSystem < SecondOrderSystem
	...
	properties
		...
		bc@double scalar = 0.1 % Cart damping
		...
		fLim@double scalar = 15 % Cart force limits
	...
	methods
		...
		%DERIVEDYNAMICS Derive the dynamics for a n-link pendulum on a cart.
			...
			% Control inputs
			f = sym('f'); ft = str2sym('f(t)');

			% Generalized coordinates and derivatives
			x = str2sym('xc'); Dx = str2sym('Dxc');	xt = str2sym('xc(t)');
			for i = 1:nLinks
				theta(i) = sym(['theta' num2str(i)]);
				Dtheta(i) = sym(['Dtheta' num2str(i)]);
				thetat{i} = str2sym(['theta' num2str(i) '(t)']);
			end % for

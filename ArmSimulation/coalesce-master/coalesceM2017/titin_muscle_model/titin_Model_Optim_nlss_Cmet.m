...
vmax = 1;%10*L0;    % [m/s] max contraction velocity ( source: Kiisa )
...
e_fit1 = 0.11; %0.15
off_fit1 = -0.08; %-0.05
%f_met_n_fit1 = 0.22*0.5.*(-(dxce/vmax-off_fit1)+((dxce/vmax-off_fit1).^2+e_fit1^2).^(1/2))./((dxce/vmax-off_fit1).^2+e_fit1^2).^(1/2);
e_fit2 = 0.2; % 0.01;
%f_met_n_fit2 = 0.01+(0.11*(dxce/vmax))*0.5.*((dxce/vmax)+((dxce/vmax).^2+e_fit2^2).^(1/2))./((dxce/vmax).^2+e_fit2^2).^(1/2);
...
nlp.addPdeConstraint(dxm, g-(Fss-Bm*Ldot)/m_r, t, 'Method',IntegrationMethod, 'Description','Dxm dynamics');

Fce = Fmax*A;

nlp.addPdeConstraint(xp, dxp, t, 'Method', IntegrationMethod,'Description','xp dot dynamics');
nlp.addPdeConstraint(dxp, ((Fss+Bss*Ldot) - (Fce + Bce*dxce + Kts*xts + Bts*dxts))/m, t, ...

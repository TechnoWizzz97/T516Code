...
Kss = 12000;%2369.7;           % [N/m] series-spring stiffness ( source: Kiisa Program )
Kts = 15000;%3754.5;            % [N/m] resistive spring stiffness ( source: Kiisa program )
Ip = 0.5*m*R^2;      % [Nm2] Inertia of pulley ( arbitrary )
Bts = 0.3145;

m_r = .25;             % Resistance mass [kg]
...
Pin = A*Fiso_max*dxce; % Mechanical Input Power
Pout = Kss*(xm-xp)*dxm; % Mechanical Output Power
COT = Pin/(m_r*g);  % Cost of Transport ( should be metabolic power when I find a good way to calculate it )
COT = COT*dxm;
...
T = Kss*(xm-xp);   % Tension response
nlp.addPdeConstraint(dxm, g-T/m_r, t, 'Method',IntegrationMethod, 'Description','Isometric pde constraint');
...
nlp.addConstraint(-vCE_max, dxce, vCE_max, 'Description','Velocity Constraint');
...
nlp.addObjective(trapz((A*Fiso_max)^2, t), 'Description','Minimize CE force^2');
%nlp.addObjective(max(abs(squeeze(eval(Pin)))), 'Description','Minimize Input Mechanical Power');
%nlp.addObjective(max(abs(squeeze(eval(Pout)))), 'Description','Minimize Output Mechanical Power');
%nlp.addObjective(trapz(A*Fiso_max*xm, t), 'Description','Minimize Mechanical Work');
%nlp.addObjective(trapz(Pin, t)/(m_r*g*trapz(dxm,t)), 'Description','Minimize Cost of Transport');
%nlp.addObjective(trapz((Kss*(xm-xp))^2, t), 'Description','Minimize Output force^2');
%nlp.addObjective(trapz(T^2, t), 'Description','Minimize Max Output Force');
...
Pinsol = squeeze(eval(Pin));
Poutsol = squeeze(eval(Pout));
